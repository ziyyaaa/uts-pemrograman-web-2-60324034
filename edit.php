...
<body>
    ...
    if (isset($_GET['id'])) {
        ...
        // ambil data berdasarkan id_kategori
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($errors['kode'])) {
            $check_sql = "SELECT id_kategori FROM kategori WHERE kode_kategori = ? AND id_kategori != ?";
            $check_stmt = $conn->prepare($check_sql);
            $check_stmt->bind_param("si", $kode, $id);
            $check_stmt->execute();
        ...
        if (empty($errors)) {
            ...
            if ($stmt->execute()) {
                ...
                $errors['global'] = "Gagal mengupdate data: " . $conn->error;
            }
            $stmt->close();
        }
    }
